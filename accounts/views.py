...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            body = render_to_string('accounts/email_verification.html', { 
                'user': user,
                'domain': current_site,
            ...
            send_email.send()
            messages.info(request, 'Enviamos un mensaje al correo:  ' + email + ' para verificar la cuenta.')
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, 'Las ceredenciales son incorrectas')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request, 'Un email fue enviado a tu correo para resetear tu password')
            return redirect('login')
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Porfavor resetea tu password')
